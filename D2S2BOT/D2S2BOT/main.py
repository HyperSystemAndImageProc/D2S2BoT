...
# lr, num_epochs, batch_size = 0.001, 50, 64 #IN
lr, num_epochs, batch_size = 0.001, 100, 32 #SV
...
for index_iter in range(ITER):
    ...
    # eval
    newnet =net.cuda()
    # newnet.load_state_dict(torch.load('./IN/11_15_09_400.9596.pt'))
    # newnet.load_state_dict(torch.load('./IN/11_15_09_400.9596.pth'))
    pred_test_fdssc = []
    ...
    # classification, oa, confusion, each_acc, aa, kappa = acc_reports(pred_test_fdssc, gt_test1)
    ...
    sio.savemat('IN_pred_proposed.mat', {'pred': pred_test_fdssc})
    sio.savemat('IN_gt_proposed.mat', {'gt': gt_test})

    overall_acc_fdssc = metrics.accuracy_score(pred_test_fdssc, gt_test1)
    confusion_matrix_fdssc = metrics.confusion_matrix(pred_test_fdssc, gt_test1)
    each_acc_fdssc, average_acc_fdssc = aa_and_each_accuracy(confusion_matrix_fdssc)
    kappa = metrics.cohen_kappa_score(pred_test_fdssc, gt_test1)

    torch.save(newnet.state_dict(), "./" +dataset+ "/" +dataset+ day_str + str(round(overall_acc_fdssc, 4)) + '.pt')
    KAPPA.append(kappa)
    ...
    generate_png(all_iter, newnet, gt_hsi,dataset, dataacc, device, total_indices,overall_acc_fdssc)
